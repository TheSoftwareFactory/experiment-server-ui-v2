...
describe('Reducer', () => {
  ...
  it('should update single app when state is empty', () => {
    ...
    expect(nextState).to.equal(fromJS(
      {
        apps : [{id: 24, rangeconstraints: [], configurationkeys: [], name: "UusiJuttu"}]
      }
    ));
  });
  it('should update single app when state is not empty and is in state', () => {
    const state = fromJS(
      {
        apps: [{id:1, name: "testiAppi"},{id:2, name: "2testiAppi2"},{id:3, name: "UusiJuttu"}]
      }
    );
    const action =
    {
